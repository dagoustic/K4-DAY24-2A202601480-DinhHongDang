...
def generate_hypothesis_questions(text: str, n_questions: int = 3) -> list[str]:
    ...
    if OPENAI_API_KEY:
        try:
            ...
            resp = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": f"Dựa trên đoạn văn, tạo {n_questions} câu hỏi ngắn mà đoạn văn có thể trả lời. Trả về mỗi câu hỏi trên 1 dòng."},
                    {"role": "user", "content": text},
                ],
                max_tokens=200,
            )
    ...
    if not sentences:
        return [f"Nội dung của {text[:30]} là gì?"]
    return [f"{s.rstrip('.')} như thế nào?" for s in sentences[:n_questions]]
    ...
    if OPENAI_API_KEY:
        try:
            ...
            resp = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": """Phân tích đoạn văn và trả về định dạng JSON:
{
  "summary": "tóm tắt 2-3 câu ngắn gọn",
  "questions": ["câu hỏi 1", "câu hỏi 2", "câu hỏi 3"],
  "context": "1 câu ngắn mô tả vị trí và chủ đề đoạn văn trong tài liệu",
  "metadata": {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}
    ...
    # Extractive fallback when API fails or no API key
    return {
